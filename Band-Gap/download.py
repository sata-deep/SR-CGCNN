#!/usr/bin/env python
"""Download a CGCNN band-gap dataset from Materials Project.

The script writes the ordinary CGCNN dataset layout into this directory:

    id_prop.csv       rows: material_id, band_gap
...
The default limit is 10000 materials, matching the formation-energy dataset.
"""
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Download Materials Project band gaps and CIFs "
                    "as a CGCNN dataset.")
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Directory for id_prop.csv, atom_init.json, and CIF files "
             "(default: this Band-Gap directory).")
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Maximum number of materials to save (default: 10000).")
    parser.add_argument(
    ...
    parser.add_argument(
        "--band-gap-min", type=float, default=None,
        help="Minimum band gap in eV.")
    parser.add_argument(
        "--band-gap-max", type=float, default=None,
        help="Maximum band gap in eV.")
    parser.add_argument(
...
def band_gap_window(args):
    if args.band_gap_min is None and args.band_gap_max is None:
        return None
    return (args.band_gap_min, args.band_gap_max)
...
def query_materials(args):
    ...
    bg_window = band_gap_window(args)
    if bg_window is not None:
        criteria["band_gap"] = bg_window
...
def write_dataset(docs, output_dir, overwrite=False):
    ...
    with id_prop_path.open("w", newline="") as csv_file:
        ...
        for doc in docs:
            ...
            band_gap = get_doc_value(doc, "band_gap")
...
def main():
    ...
    print("Target property: band_gap (eV)")
